...
//表单验证========================================================
		...
		function validate(val){
			...
			if(!acc){
				val = val.trim();
				if(!val){
					$('#tip01').html('<span class="tip_warning">请输入账号</span>').fadeIn(100);
					return flag;
				}
			}else if(acc.length<5){
				$('#tip01').html('<span class="tip_warning">账号长度过短</span>').fadeIn(100);
				return flag;
			}else{
				$.ajax({
					type : 'POST',
					...
					success : function(data) {
						if (data.code == 1) {
							$('#tip01').html('<span class="tip_succ">账号可用</span>').fadeIn(100);
						} else {
							$('#tip01').html('<span class="tip_warning">账号已存在</span>').fadeIn(100);
							setTimeout(function(){
								$('#register_acc').val('');
							},1000);
						}
		...
		function sign(flag){
			...
			type = 'sign';
			formId = '#signForm';
			var acc = $('#register_acc').val().trim();
			var pass = $('#register_password').val().trim();
			var phone = $('#register_phone').val().trim();
			var mail = $('#register_mail').val().trim();
			var name = $('#register_name').val().trim();
			if(validate(acc)&&varifyPass(pass)/*&&checkMobile(phone)*/&&checkMail(mail)){
				$.ajax({
					type : 'POST',
					url : url,
					dataType : 'json',
			...
			$.ajax({
				...
				success : function(data) {
					if (data.code == 1) {
							...
							swal("好像哪里有不对呢", "账号或密码有不正确的哦", "error");
					}
				}
			});
		}
...
//动画旋转===========================================================================================
      ...
      //动画旋转
		...
		//注册成功提示===========================================
		function signSucc(){
			swal("账号注册成功", "「		每一个新成员的加入，我们都倍感欣慰	」", "success");
			return true;
		};
